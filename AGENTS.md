...
- 状态:**六周全部完成并通过验收;`v1-week1`~`v1-week6` 六个标签已全部补打(2026-07-25,本地),尚未 `push` 到远程。V1(2D 版本)收官,V1.5(深化)进行中——方向①(武器/伤害深化)、方向②(敌人 AI 深化)、`V1.5-cleanup` 与 `V1.5-debug` 均已完成。V1.5-3 的完整教程与未来版 `Reference/` 已准备,步骤 1～5 已落地并通过相应的 Play、批量验证与代码 review;当前已具备 Seed 地图、实体房间/通道、房间战斗闭环和混合相机,下一步实施步骤 6 的六类房间内容。**
- 已有内容:
...
- **V1.5-2 敌人 AI 深化**(2026-08-11):**已完成并通过 Play 验收**。结构整理后 `Assets/Scripts/AI/` 当前 26 个脚本(`Framework/` 通用行为树、`Enemy/` 通用敌人节点、`Boss/` 阶段与技能节点),另新增 `Entities/EnemyBehaviour.cs` + `EnemyBrain.cs`、`StateMachines/Enemy/EnemyFreeState.cs`、`Weapons/Projectiles/EnemyProjectile.cs`;`EnemyController` 已改成“行为树负责主动决策 + FSM 只负责 Free/Knockback/Dead 被动打断”,旧 `EnemyPatrolState`/`EnemyChaseState`/`EnemyAttackState` 已删除。已创建 `MeleeBehavior`/`RangedBehavior`/`BomberBehavior`/`BossBehavior` 四份 SO、`EnemyMeleed`/`EnemyRanged`/`EnemyBomber`/`EnemyProjectile` 预制体,并更新 `Boss.prefab` 与房间配置。最终行为包括近战、保持距离射击、自爆冲脸、Boss 半血切远程灼烧技能;视野采用每敌人 Blackboard 感知快照并恢复进入/退出双半径迟滞;Boss 阶段阈值来自 SO。真实代码继续使用英式拼法 `EnemyBehaviour`/`BehaviourTree`;选择节点文件为 `AI/Framework/SelectorNode.cs`,近战预制体当前名为 `EnemyMeleed`,不要擅自复制美式拼法平行类型。完整实际完成记录见 `devlog/V1.5-2.md` 第 10 节。
...
- **V1.5-3 步骤 3：实体房间与通道**(2026-08-26):**已落地 `Assets/` 并通过 Play 与代码 review**。新增 `Level/Building/` 下的 `DungeonBuilder`、`RoomCatalog`、`RoomConnection`,把纯数据图建造成六类实体房间及东/北唯一生成的通道;Door 已从传送器改为可开关的实体 Gate,玩家出生在 Start 房。完成灰盒房间和横/纵通道 Prefab,同 Seed 的世界布局与内容选择一致。静态编译为 **0 error / 0 项目代码 warning**;已由用户提交为 `0951590`。完整记录见 `devlog/V1.5-3.md`。
...
- 下一步:**V1(2D 核心玩法)收官,进入 V1.5(深化阶段)**,而不是立刻做 V2(3D 化)——用户判断"核心玩法刚搭起来,3D 化为时过早",且学习目标是把游戏编程模式吃透,武器系统这类还有明显深挖空间。V2/V3 推迟为更后面的大版本,`EventBus`/`ICommand`/`IWeaponStrategy`/`RoomConfig` 这几层设计上不认识 2D Sprite/物理,理论上可原样迁移,不会因为推迟而过时。
  ...
  - **当前状态**:方向①、方向②与 `V1.5-debug` 均已完成并通过验收。方向③的教程/Reference 与步骤 1～5 均已完成,下一步按 `devlog/V1.5-3.md` 实施步骤 6 的六类房间内容,随后完成步骤 7 的三选一局内强化并再次提交。方向④存档系统继续排在其后。
...
## 当前代码结构快照(V1.5-3 步骤 5 完成,2026-08-27)

> "代码实际长什么样"的速查表,方便新对话快速定位。真实进度以 `Assets/` 为准;下面每条都对应已经落地的文件。
> `Reference/Scripts/` 从 2026-08-19 起已提前写成 V1.5-3 的未来完成态,不再与本节描述的 `Assets/Scripts/` 102 个现行脚本镜像;代码审查和进度判断必须区分两者。

**脚本清单(`Assets/Scripts/`,当前共 102 个 `.cs`;可正常编译)**
...
- `Core/`
  ...
  - **`GameEvents.cs`**(第 3 周起,逐周扩充)——`readonly struct` 事件集中定义:血量/弹药/武器/技能冷却(3~4 周)、关卡事件(5 周起)、**战斗反馈 3 个(6 周):`EnemyDamagedEvent(Position,Damage)`/`EnemyDiedEvent(Position)`(语义事件)、`ScreenShakeEvent(Intensity,Duration)`(表现指令)**,以及步骤 1 新增的 `RunStartedEvent`/`RunEndedEvent`/`RunUpgradeSelectedEvent`。`RoomType` 已扩展为 Start/Normal/Elite/Treasure/Recovery/Boss,关卡快照使用 `RoomMapData[]` 与稳定 `RoomId`;旧 `DoorEnteredEvent` 只为固定三房兼容而暂时保留。**标识用枚举不用字符串;枚举/`RoomType` 放 `Core` 因为底层不能依赖上层**。
  - **`RunManager.cs`**(V1.5-3 步骤 1)——单局生命周期入口:确定/复用 Seed,统计探索、击杀与强化,监听玩家死亡,统一发布胜负结算,控制最终输入禁用和场景重载。`[DefaultExecutionOrder(-500)]` 保证关卡读取 Seed 前已经初始化。
...
- **`AI/`**(V1.5-2 新建,命名空间 `Game.AI`,当前 26 个 `.cs`,**已 review 并通过 Play 验收**)
  - `AI/Framework/`:通用框架 `NodeState` + `Node`,组合节点 `SequenceNode`/`SelectorNode`,装饰节点 `InverterNode`/`CooldownNode`,叶节点基类 `ConditionNode`/`ActionNode`,以及 `Blackboard`/`BehaviourTree`。这些都是纯 C# 决策逻辑;教程演示用 `BehaviourTreeTester` 已从生产代码删除。
  - `AI/Enemy/` 11 个通用敌人节点:视野/攻击/射击/自爆范围条件,追击/巡逻/近战/保持距离/射击/自爆动作。`BossPhaseCondition` 与 `BossSkillAction` 位于 `AI/Boss/`。
...
- `Entities/`
  ...
  - `PlayerController.cs`——玩家状态机"上下文":持有 `Rb`/`health`/`SpriteRenderer`/`MoveInput` + 5 个状态实例 + `stateMachine`;`Update` 只做**冷却计时 + `Tick`**,`FixedUpdate` **先 `Rb.linearVelocity = Vector2.zero` 再跑 `FixedTick`**。输入和屏幕坐标换算统一由 `PlayerInputHandler` 完成,再通过 `SetMoveInput(v)` 与 `SetAimDirection(v)` 写入;其他外部入口包括 `TriggerDash()`(由 `DashCommand` 调)、`TriggerAttack()`(由 `WeaponController` 近战时调)。只读暴露 `CanAct`(仅 Idle/Move 为真)、`CanDash`。**它还负责把 `health.HealthChanged` 桥接成全局 `PlayerHealthChangedEvent`**(`Start` 广播一次初始血量),并在 `StartDashCooldown()` 里广播 `SkillCooldownStartedEvent(SkillId.Dash, ...)`。
...
- `Weapons/`(第 3 周重构成"数据 + 策略";整理后按职责拆分子目录)
  ...
  - `Strategies/IWeaponStrategy.cs`——V1.5-1 后签名为 `void Fire(WeaponController controller, DamageInfo damageInfo)`。**策略无状态**,冷却/弹药由 controller 管,SO→伤害包只由 controller 翻译。具体策略与三个 Decorator 也位于 `Strategies/`。
  - `Strategies/RangedWeaponStrategy.cs`——从 `controller.BulletPool` 取子弹、按 `FirePoint` 朝向发射,并 `bullet.SetDamageInfo(damageInfo)`。
  - `Strategies/MeleeWeaponStrategy.cs`——角色前方 `range` 处 `Physics2D.OverlapCircle(点,半径,filter,复用缓冲区)`(第 6 周从 `OverlapCircleAll` 改成零分配版),圈内 `Enemy` 扣血。**filter 用 `ContactFilter2D.noFilter`**(Unity 6:`NonAlloc`/`NoFilter()` 都已废弃,分别改成 `OverlapCircle` 重载 / 静态属性 `noFilter`)。缓冲区固定 16,超出被无视。
  ...
  - `Projectiles/Bullet.cs`——`IPoolable` 玩家子弹,`FixedUpdate` 用 `linearVelocity` 前进 + 计时回收,命中敌人后同时走 `Health.TakeDamage(DamageInfo)` + `StatusEffectManager.ApplyEffects`,再回池;主要注入入口是 `SetDamageInfo`,旧 `SetDamage(int)` 仍作兼容。
  - **`Projectiles/EnemyProjectile.cs`**(V1.5-2)——池化敌人子弹,`Launch` 注入速度与 `DamageInfo`;命中 Player 后造成伤害并施加状态异常。类名与预制体序列化标识均已统一为 `EnemyProjectile`。
  - `AmmoPickup.cs`——`OnTriggerEnter2D` 碰到 `Player` → `WeaponController.AddAmmo(amount)` → `Destroy`。无限弹药武器(近战)会被 `AddAmmo` 直接跳过。
  - **`Skills/Grenade.cs`**(第 4 周)——`IPoolable` 手雷。`OnEnable` 给初速 + 阻尼,引信烧完 `Explode()` 后造成范围伤害,再显示爆炸圈并计时回池。
  - **`Skills/GrenadeThrower.cs`**(第 4 周,挂 Player)——只管冷却 + 从池里取一颗手雷丢出去;`Throw()` 里广播 `SkillCooldownStartedEvent(SkillId.Grenade, cooldown)`。
- **`Commands/`**(第 4 周新建,命名空间 `Game.Commands`)
  ...
  - `PlayerInputHandler.cs`(挂 Player)——**全项目唯一读键盘鼠标的地方**。**持续动作**(移动/按住左键连发)每帧直接 `Execute()`,**不入队**;**离散动作**(闪避 `Shift` / 手雷 `Q`)`buffer.Enqueue(...)` 排队;**切枪**(1/2/3)立即执行。`Update` 末尾 `buffer.Tick()`;`OnDisable` 清零移动并清空缓冲,避免奖励/结算禁用输入后残留动作。
- `UI/`(第 3 周新建)
...
- **`Level/`**(第 5 周新建,命名空间 `Game.Level`)
  ...
  - `Building/`(V1.5-3 步骤 3)——`DungeonBuilder` 按图实例化房间与通道,`RoomCatalog` 依据房型从候选配置中确定性抽取内容,`RoomConnection` 维护方向、Gate 与通道锚点。通道只由东/北方向创建一次,避免无向边重复实例化。
  - `Rooms/RoomConfig.cs`——房间 SO(`Create > Game > Room Config`):`roomName`/`type`/`enemySpawns[]`/`pickupSpawns[]`/`clearRewardSpawns[]`。生成位置用**相对房间中心的 `localPosition`**;进入时 Pickup 与清场后 Pickup 分开配置。同文件有 `EnemySpawn`/`PickupSpawn` 两个 `[System.Serializable] struct`。资产在 `Assets/Data/Rooms/`。
  ...
  - `Rooms/Room.cs`——实体房间运行时主体。`RoomEncounterTrigger` 转发玩家进入/离开;首次进入按 `config` 生成敌人与即时 Pickup,有存活敌人时锁住所有相连 Gate。它订阅每个敌人的 `Health.Died` 计数(不轮询),清空后生成 `clearRewardSpawns`、发布局部 `RoomCleared` 并开门。`spawned`/`IsCleared` 保证重访不重复生成;空房会立即完成流程。
  - `Rooms/Door.cs`——只负责启用/禁用自己的 Collider,是战斗期间的实体封锁,不再承担传送与房间寻址。
  - `LevelManager.cs`——从 `RunManager` 取得 Seed,生成并建造地图,注册所有运行时房间的局部事件并桥接成全局房间事件。进入房间时切至该房的固定 RoomCamera,离开房间进入通道时切回 ExplorationCamera;Boss 房清空后通知 `RunManager` 胜利结算。
- **`Pickups/`**(V1.5-3 步骤 4)
  - `AmmoPickup.cs`/`HealthPickup.cs`——分别为当前武器补弹和恢复生命;只有资源确实增加时才销毁,满弹、无限弹药、满血或死亡状态下均保留。
- `StateMachines/`
...
- 当前阶段:**六周全部完成,V1(2D)收官,V1.5 方向①、方向②、`V1.5-cleanup` 与 `V1.5-debug` 均已验收完成;V1.5-3 教程/Reference 与步骤 1～5 均已完成**。六篇 `devlog/week1~6.md` 均已写完实际完成记录;`v1-week1`~`v1-week6` 六个标签已本地补打(未 push)。
- **下一步**:先提交步骤 4～5,再按 `devlog/V1.5-3.md` 完成步骤 6 的六类房间内容与步骤 7 的三选一局内强化,通过 Play 和代码 review 后再提交一次;V1.5-3 完成后再做存档系统。README 规划的 V2(3D 化)、V3(联机)推迟到 V1.5 之后。
...
| 状态机 + 行为树 | 玩家使用状态类 FSM;敌人使用行为树主动决策 + FSM 被动打断 | 不要恢复敌人 Patrol/Chase/Attack 状态;击退和死亡继续由 FSM 抢占 |
