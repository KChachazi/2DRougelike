...
Start 房 ─ Normal 房 ─ 分岔/转折 ─ Normal 房 ─ Boss 房
                        ...
                        └────────── Normal 房
   │
   ├─ 首次进入战斗房：封门 → 生成敌人 → 清空 → 生成奖励 Pickup 并开门
   └─ 返回已清房间：不重复刷怪、不再次锁门
                              │
               ┌──────────────┴──────────────┐
               ▼                             ▼
...
| 7 | 三选一局内强化 | 否，后续深化 |
| 8 | 二维拓扑小地图与可配置可见性 | 否，后续深化 |

后半部分代码仍按未来完整形态编写，因此步骤 2～4 的 Reference 可能提前包含特殊房标签、一次性 Pickup 或更严格的生成约束。它们可以保留，方便以后继续深化；但在当前基本单局阶段，**本节定义的核心验收范围优先**，不能因为尚未实现奖励、特殊房或二维小地图而阻塞地图生成主流程。
...
namespace Game.Core
{
    ...
    /// <summary>指定 Id 的房间已完成战斗，或已首次激活无战斗内容。</summary>
...
## 步骤 4：房间触发、战斗门、一次性内容与重访

**这一步做什么**：把“地图能走”变成“房间流程能玩”。首次进入战斗房时生成敌人并封门，最后一名敌人死亡后生成清场奖励并同时开门；恢复房和宝箱房在首次进入时生成 Pickup。离开后再进入仍然会报告房间进出，但不重复生成、不重新锁门、不重复发奖。

这里的核心流程是：

```text
检测进入
  → 始终发送 Entered（相机、小地图和统计仍需要）
  → 判断是否已生成过内容
      → 是：结束，这是重访
      → 否：生成首次内容
          → 有敌人：锁门 → 等待全部死亡 → 生成清场 Pickup → 开门
          → 无敌人：生成恢复/宝箱 Pickup → 直接记为已清理
检测离开
  → 发送 Exited，但不销毁未拾取的 Pickup，也不重置房间
```

需要特别区分两层语义：

- `RoomType` 决定生成器从哪一类 `RoomConfig` 中选内容，以及 Boss 清空后是否结束本局。
- `Room` 不写一个巨大的类型 `switch`；它根据实际配置的 `enemySpawns`、`pickupSpawns` 和 `clearRewardSpawns` 执行通用流程。这样即使将来出现“带敌人的宝箱房”，也不需要改写房间主逻辑。

> 步骤 3 中的 `Room`/`LevelManager` 代码是实体建造阶段的中间快照。从本步起，以下面的房间生命周期和 `Reference/` 最终版为准。
...
- 不再额外保存 `locked`/`IsLocked`：当前没有调用方需要查询它，`Collider2D.enabled` 已经是真正的物理状态，避免同时维护两份可能失去同步的数据。
...
### 4.3 用两个 Pickup 阵列表达不同生成时机

`pickupSpawns` 保留为“首次进入就生成”，另外增加 `clearRewardSpawns` 表示“战斗清空后才生成”：

```csharp
[Header("房间内容")]
[Tooltip("敌人出生点（相对房间中心）")]
public EnemySpawn[] enemySpawns;
[Tooltip("首次进入房间时生成的拾取物，用于恢复房和宝箱房")]
public PickupSpawn[] pickupSpawns;
[Tooltip("战斗清空后生成的一次性奖励，用于普通房和精英房")]
public PickupSpawn[] clearRewardSpawns;
```

删除这一版不再使用的 `healAmount` 和 `grantsUpgradeReward`。恢复、弹药和强化都由实际 Pickup 表达，不再让 `RoomConfig` 同时保存“自动恢复数值”与“恢复物预制体”两份来源。

### 4.4 重写 `Assets/Scripts/Level/Rooms/Room.cs`

这是房间生命周期的主体，修改规模较大，因此给出完整代码：
...
namespace Game.Level
{
    /// <summary>一个实体房间：只生成一次内容，管理战斗门、清场奖励和固定相机。</summary>
    public sealed class Room : MonoBehaviour
    {
        [SerializeField] private RoomConfig config;
        [SerializeField] private Transform playerSpawnPoint;
        ...
        private void SpawnInitialContents()
        {
            if (config == null) return;
            Transform parent = contentParent != null ? contentParent : transform;
        ...
        private void SpawnPickups(PickupSpawn[] spawns, Transform parent)
        {
            if (spawns == null) return;
            foreach (PickupSpawn spawn in spawns)
            {
                Vector3 worldPosition = transform.position + (Vector3)spawn.localPosition;
                EnemyFactory.Create(spawn.prefab, worldPosition, parent);
            }
...
`contentsSpawned` 只限制内容初始化，不限制 `Entered`/`Exited`。这正是“房间不重复刷新，但仍然可以重访”的关键。

`CompleteEncounter()` 的顺序是“标记完成 → 生成清场奖励 → 发送清房事件 → 开门”。奖励 Pickup 会留在 `ContentRoot` 下，玩家可以立即拾取，也可以离开后回来拾取；房间不会再生成第二份。

同时从 `LevelManager` 删除以下旧责任：

- `RunUpgradeManager upgradeManager` 字段；
- `RewardRequested` 的订阅、退订和 `OnRewardRequested()`；
- `OnRoomEntered()` 中的 `TryConsumeRecovery()` 自动回血分支；
- `Game.Entities` 与 `Game.Rewards` 两个不再使用的 `using`。

`LevelManager.OnRoomEntered()` 回到单一职责：
...
### 4.5 建立一次性恢复/弹药 Pickup

将旧 `Assets/Scripts/Weapons/AmmoPickup.cs` 移到 `Assets/Scripts/Pickups/AmmoPickup.cs`，命名空间改为 `Game.Pickups`。同目录新建 `HealthPickup.cs`。两份都给出完整代码。
...
{
    /// <summary>为玩家当前武器补充弹药；无法补充时保留在房间中。</summary>
    [RequireComponent(typeof(Collider2D))]
    public sealed class AmmoPickup : MonoBehaviour
    {
        [SerializeField, Min(1)] private int amount = 15;

        private void OnTriggerEnter2D(Collider2D other)
        {
            if (!other.CompareTag("Player")) return;
            if (!other.TryGetComponent(out WeaponController weapon)) return;
            if (!weapon.TryAddAmmo(amount)) return;
            Destroy(gameObject);
        }
    }
}
...
为了让弹药已满或当前武器无限弹药时不浪费 Pickup，把 `WeaponController.AddAmmo()` 替换为下面这个完整方法：
...
Pickup Prefab 的 Collider2D 必须勾选 `Is Trigger`。对于恢复物，满生命时不销毁；对于弹药物，当前武器无法接收弹药时也不销毁。玩家可以受伤或切换武器后再回来拾取。

### 4.6 完整房间状态表

| 场景 | `contentsSpawned` | `IsCleared` | Gate | 结果 |
|---|---:|---:|---|---|
| 首次进入有敌房 | false → true | false | 锁 | 生成敌人和进入时 Pickup，等待全部死亡 |
| 最后一名敌人死亡 | true | false → true | 开 | 生成 `clearRewardSpawns`，发清房事件并开门 |
| 首次进入恢复/宝箱房 | false → true | false → true | 开 | 生成 `pickupSpawns`，Pickup 自己处理拾取效果 |
| 离开时未拾取 | true | true | 开 | Pickup 仍然留在运行时房间中 |
| 重访已清房 | true | true | 开 | 仍发 `Entered`，但不刷怪、不生成第二份 Pickup、不重复发清房事件 |
| 首次进入空配置房 | false → true | false → true | 开 | 立即完成，不会被永久锁住 |

这里有三个容易漏掉的沉默问题：

1. **不能在首次进入时就生成清场奖励**。`pickupSpawns` 和 `clearRewardSpawns` 必须分开，否则玩家可以在战斗结束前拿走奖励。
2. **不能在离开时重置 `contentsSpawned`**。否则重访会重复刷怪和刷奖。
3. **敌人事件必须退订**。房间销毁时仍存活的敌人可能持有 `OnEnemyDied` 委托；`OnDestroy` 统一解除，避免跨场景残留。

### 4.7 建议的 Play 验收顺序

1. 进入普通战斗房：只封锁真正连通的 Gate，没连接的方向仍是墙。
2. 尝试撞门：玩家不能穿过。
3. 杀到只剩一名敌人：门仍锁，清场奖励尚未出现。
4. 杀死最后一名：清场 Pickup 生成，全部连接门同时打开。
5. 离开而不拾取奖励，然后返回：原 Pickup 仍在，不会再生成第二份。
6. 重访战斗房：不再刷怪，不再锁门，但 `Entered`/`Exited` 仍正常发生。
7. 满生命碰到 HealthPickup：不消失；受伤后再拾取才恢复并销毁。
8. 弹药已满或当前是无限弹药武器：AmmoPickup 不消失；切到可补充武器后才能拾取。
9. 清空一个敌人数组为空的特殊房：不会卡死。

> ✅ **步骤 4 验收标准**：首次进入、战斗封门、最后敌人死亡、清场奖励生成、开门、特殊房 Pickup 和重访全部正确；不依赖房间顺序；从任意连接方向进入都得到相同行为。
...
namespace Game.Level
{
    /// <summary>描述房间类型、选择规则，以及首次进入和清场时生成的内容。</summary>
...
| 资产建议名 | Type | Depth | 进入时 Pickup | 清场 Pickup | 内容建议 |
|---|---|---:|---|---|---|
| `Room_Start_01` | Start | 0～0 | 无 | 无 | 空房 |
| `Room_Normal_Early_01` | Normal | 1～3 | 无 | 小额生命/弹药 | 2 近战 |
| `Room_Normal_Mid_01` | Normal | 2～99 | 无 | 小额生命/弹药 | 近战 + 远程 |
| `Room_Normal_Bomber_01` | Normal | 3～99 | 无 | 小额生命/弹药 | 远程 + 自爆 |
| `Room_Elite_01` | Elite | 2～99 | 无 | 强化 Pickup | 更密集混合编队 |
| `Room_Treasure_01` | Treasure | 0～99 | 强化 Pickup | 无 | 空房，奖励可以暂不拾取 |
| `Room_Recovery_01` | Recovery | 0～99 | 生命 + 弹药 | 无 | 空房，拾取物只生成一次 |
| `Room_Boss_01` | Boss | 4～99 | 无 | 无 | Boss 死亡后结束本局 |
...
> ✅ **步骤 6 验收标准**：六种房间都能在若干 Seed 中出现；Start 为空、Boss 最远、宝箱/恢复是叶节点；深度限制生效；普通/精英房的奖励只在清场后生成，恢复/宝箱房的 Pickup 只在首次进入时生成；同 Seed 选择同一批 RoomConfig；没有“配置缺失”的 Error 或回退 Warning。

---

## 步骤 7：最小局内强化闭环

**这一步做什么**：让宝箱房或清场奖励中的 `UpgradePickup` 在被拾取时打开三选一；强化只在当前 Run 生效，并复用现有 `DamageInfo`、状态异常和玩家能力边界。房间不直接打开 UI，`LevelManager` 也不再中转强化奖励。
...
namespace Game.Rewards
{
    ...
    public sealed class RunUpgradeManager : MonoBehaviour
    {
        ...
        public bool TryPresentChoices()
        {
            if (presenting)
            {
                GameDebug.Warning(DebugCategory.Level, "奖励 UI 已经处于选择中，暂不消耗新的强化拾取物。", this);
                return false;
            }
...
- 应用强化、广播统计事件，然后恢复玩家输入。

如果强化池全部叠满，`TryPresentChoices()` 返回 `true` 并消耗这份 Pickup，但不再打开空 UI。如果 UI 引用缺失或已有一次选择正在进行，则返回 `false`，Pickup 保留，不会静默丢失奖励。

### 7.4 新建 `Assets/Scripts/Pickups/UpgradePickup.cs`
...
namespace Game.Pickups
{
    /// <summary>被玩家拾取后打开一次局内强化选择。</summary>
    [RequireComponent(typeof(Collider2D))]
...
`UpgradePickup` 只负责把一次物理拾取转成“尝试打开强化选择”。房间不需要认识 `RunUpgradeManager`，`RunUpgradeManager` 也不需要认识是宝箱房还是精英房生成了它。

### 7.5 小范围更新 `PlayerInputHandler.cs`
...
### 7.6 新建 `Assets/Scripts/UI/RewardChoiceUI.cs`
...
7. 创建 `UpgradePickup.prefab`：根物体添加 SpriteRenderer、`Is Trigger` 的 Collider2D 和 `UpgradePickup`。
8. 宝箱房把 `UpgradePickup` 放进 `pickupSpawns`；精英房或高难普通房把它放进 `clearRewardSpawns`。
9. `LevelManager` 不再需要 `upgradeManager` 引用；强化入口只在 Pickup 上。

> ✅ **步骤 7 验收标准**：进入宝箱房或清空配有强化奖励的战斗房后出现 `UpgradePickup`；不碰它时可以离开并返回，且不会重复生成；拾取后三选一出现、输入禁用，点击后 UI 消失且输入恢复；相同 Seed 和相同拾取顺序的候选顺序一致；七种强化都能通过数值或战斗表现验证；重开场景后所有叠层归零。

---

## 步骤 8：二维小地图与最终整体验收

**这一步做什么**：把旧的一排房间格子升级为真实二维拓扑，显示连接、房间类型、当前房和清空状态；地图可见性保留为可配置接口，本版本默认展示完整拓扑。
    ...
    public sealed class MinimapUI : MonoBehaviour
    {
        ...
        private void OnLevelStarted(LevelStartedEvent e)
        {
            ...
            for (int i = 0; i < e.Rooms.Length; i++)
                if (initVisible || e.Rooms[i].Type == RoomType.Start) discovered.Add(e.Rooms[i].Id);
            Refresh();
        }
...
- `initVisible = true` 时开局展示全部房间和连接，这是当前版本采用的默认方案；
- `initVisible = false` 时开局只发现 Start，进入房间后再发现当前房和它的直接邻居；
- 未发现房间和连接会隐藏，保留该接口供后续重新讨论探索迷雾时使用；
...
- [ ] 最后一名敌人死亡才结束战斗。
- [ ] 最后一名敌人死亡后生成清场 Pickup 并同时开门。
- [ ] 恢复/宝箱房只在首次进入时生成 Pickup；未拾取内容可以留到重访。
- [ ] 重访房间仍有进出事件，但不刷怪、不生成第二份奖励或恢复物。
- [ ] 可以自由回到分岔点，再选择另一条路线。
...
- [ ] 默认全图可见时，所有房间与连接完整显示；关闭 `initVisible` 后，相邻发现逻辑仍可用。
...
Assets/Scripts/Pickups/UpgradePickup.cs
...
V1.5-3 完成后总脚本数从当时的 89 增至 107，因为删除 `VictoryUI.cs` 抵消一个新增文件：`89 + 19 - 1 = 107`。`AmmoPickup.cs` 只移动目录，不改变总数。
...
Assets/Prefabs/Pickups/AmmoPickup.prefab
Assets/Prefabs/Pickups/HealthPickup.prefab
Assets/Prefabs/Pickups/UpgradePickup.prefab
...
检查 `Room.HandlePlayerEntered()`：`if (contentsSpawned) return;` 必须在 Spawn/Lock 之前。重访不能重复执行首次进入流程，但 `Entered` 必须在这个判断之前发出。
...
8. **奖励以一次性 Pickup 落地**：房间只决定何时生成奖励，Pickup 自己执行恢复、弹药或强化效果；先证明局内成长，再谈稀有度、协同和构筑流派。
...
- 已把未来完成态的完整参考代码写入 `Reference/Scripts/`；2026-08-26 按“房间生成一次性 Pickup，Pickup 自己执行恢复/弹药/强化”的最终设计修订后，Reference 当前共 107 个 `.cs` 文件。
- 准备阶段 review 发现并修正一个输入残留风险：奖励/结算禁用 `PlayerInputHandler` 时，现在会清零持续移动方向并清空输入缓冲。
- 已用 Reference 的 107 个脚本替换 Unity C# 工程的编译输入执行静态验证，结果为 **0 error / 0 项目代码 warning**（40 条 warning 均来自 Unity Package）；临时构建规则已删除，没有修改 `Assembly-CSharp.csproj`。
...
### 步骤 3 实际完成记录（2026-08-26）

- 新增 `Assets/Scripts/Level/Building/` 下的 `DungeonBuilder`、`RoomCatalog` 和 `RoomConnection`，把纯数据 `LevelGraph` 建造为实体房间、四向连接和东/北唯一生成的通道。
- 重写当前 `Room`、`Door` 与 `LevelManager` 的实体地图边界：玩家出生在 Start 房，不再使用 Door 传送；Gate 改为可开关的实体 Collider。
- 建立六类 `RoomConfig` 和 `RoomCatalog.asset`，三份 Normal 配置可供内容随机选择；Start、Elite、Treasure、Recovery 和 Boss 的类型、GUID 与 Catalog 引用均已核对。
- 完成灰盒 `Room.prefab`、`HorizontalCorridor.prefab` 与 `VerticalCorridor.prefab`；四个 CorridorAnchor 的局部坐标归零，`DungeonRoot` 保持世界原点，房间间距、门洞和通道长度一致。
- 用户已在 Play 模式确认 9～12 间真实房间与通道正常连通，同 Seed 的世界布局和内容选择一致；未完成的 EncounterTrigger 在此阶段保持禁用。
- 代码 review 确认旧传送事件和 `Door.locked` 冗余状态已删除；静态编译为 **0 error / 0 项目代码 warning**，步骤 3 完成后 `Assets/Scripts/` 共 100 个 `.cs` 文件。
- 步骤 3 已由用户提交为 `0951590 V1.5-3 程序化关卡生成: 完成实体房间与通道建造`。下一步进入步骤 4，实现房间触发、战斗门、一次性 Pickup 与重访流程。

### 步骤 4～5 实际完成记录（2026-08-27）

- 新增 `RoomEncounterTrigger`，只负责把 Player 的进入/离开触发转交给所属 `Room`；`Room` 使用 `spawned` 与 `IsCleared` 保证房间内容只在首次进入时生成，离开后重访不会再次生成敌人、奖励或重新封门。
- 普通、精英和 Boss 战斗房会在首次进入后生成敌人并锁住全部已连接 Gate；`Room` 订阅每个敌人的 `Health.Died` 事件计数，全部消灭后生成 `clearRewardSpawns`、发布清场事件并开门。空战斗配置会立即完成，不会把玩家永久关住。
- 即时 Pickup 与清场奖励已在 `RoomConfig` 中拆分为 `pickupSpawns` 和 `clearRewardSpawns`。普通/精英房的恢复奖励已移入清场数组，避免进房时提前拾取；恢复房与奖励房仍可在首次进入时直接生成各自内容。
- 新增 `Assets/Scripts/Pickups/HealthPickup.cs`，并把 `AmmoPickup.cs` 连同原 `.meta` 移至 `Pickups/`、命名空间改为 `Game.Pickups`。`WeaponController.AddAmmo()` 现在返回是否实际增加弹药，因此满弹或无限弹药时不会误消耗补弹 Pickup；满血时 HealthPickup 同样保留。
- 混合相机流程已接入：进入房间时关闭通道 ExplorationCamera、启用该房间的固定 RoomCamera；离开房间进入通道时反向切换。当前实现让 ExplorationCamera 跟随玩家并使用更大的正交尺寸,以便在通道中提供连续移动和更宽视野。
- 用户已在 Play 模式确认首次进房、封门、敌人清空、奖励生成、开门、离房重访与房间/通道相机切换均正常。代码 review 进一步修正了普通/精英房奖励生成时机、`AmmoPickup.prefab` 的旧脚本类型标识,并移除了横/纵通道 Prefab 上多余的 `Room` 组件。
- 提交前 `dotnet build Assembly-CSharp.csproj --no-restore --no-incremental` 为 **0 error / 0 项目代码 warning**（40 条 warning 均来自 Unity Package）；步骤 5 完成后 `Assets/Scripts/` 共 102 个 `.cs` 文件。下一步进入步骤 6 的六类房间内容,随后完成步骤 7 的三选一局内强化。

### 步骤 6 实际完成记录（2026-09-02）

- `RoomConfig` 已加入 `minDepth`、`maxDepth` 与 `selectionWeight`，`RoomCatalog` 会先按房型取池，再从支持当前深度的配置中确定性加权选择；没有深度候选时仍沿用既有回退和 Warning。
- 八份房间配置已按 `Room_Start_01`、三份 Normal、Elite、Treasure、Recovery、Boss 统一命名，移动时保留原 `.meta` GUID；`RoomCatalog.asset` 的全部引用继续有效。
- 三份 Normal 分别覆盖早期近战、中期近战/远程与后期自爆组合；普通和精英房奖励均在 `clearRewardSpawns`，宝箱和恢复房内容在 `pickupSpawns`。恢复房曾重叠的 Pickup 已拆开。
- Boss 房保留进入时生成的战前生命与弹药补给；这是当前版本相对教程矩阵的主动内容调整。Boss 节点的实际最小深度仍由 `LevelGenerationSettings` 与图验证器保证。

### 步骤 7 实际完成记录（2026-09-02）

- 新增 `Rewards/RunUpgradeData`、`RunModifierSet`、`RunUpgradeManager`,`UI/RewardChoiceUI` 和 `Pickups/UpgradePickup` 五个脚本；`Assets/Scripts/` 当前共 107 个 `.cs` 文件。
- 七份 `RunUpgradeData` 已覆盖伤害、武器冷却、移动速度、最大生命、灼烧、冰冻与击退,并写明可直接验收的数值描述；Icon 暂时为空时,对应 UI Image 会自动隐藏。
- Player 上的 `RunModifierSet`、`RunUpgradeManager`、`PlayerController` 与 `WeaponController` 位于同一 GameObject。伤害包、武器冷却、移动速度与最大生命扩展均已接入现有能力边界,没有让房间或 UI 直接修改战斗组件。
- RewardPanel 保持由 Canvas 根物体上的 `RewardChoiceUI` 控制,三个 Slot 的 Button/Icon/Title/Description 引用完整；`RunUpgradeManager` 的七份资产与 UI 引用完整,显示期间禁用输入,选定后恢复。
- `UpgradePickup.prefab` 的 SpriteRenderer、Trigger Collider 与脚本组件完整；宝箱房在首次进入时生成强化,精英房在清场后生成强化,两者都沿用 Room 的一次性生成与重访规则。
- 强化候选使用与地图/内容分离的 Seed 随机流；同 Seed、相同拾取与选择进程下候选顺序可复现。场景和新 Prefab/SO 的序列化引用已经逐项核对。
- 相邻修正：DebugOverlay 面板现在由始终激活的 Canvas 根组件按 `DebugEnabled && OverlayEnabled` 开启,避免把控制脚本挂在被关闭的面板自身。
- 最终静态编译 `dotnet build Assembly-CSharp.csproj --no-restore --no-incremental` 为 **0 error / 0 项目代码 warning**（40 条 warning 均来自 Unity Package）。下一步进入步骤 8 的二维小地图与整局最终验收。

### 步骤 8 实际完成记录（2026-09-02）

- `MinimapUI` 已从旧横排图标重写为依据 `RoomMapData.GridPosition` 绘制的二维拓扑；房间与连接分层生成，东/北邻接各绘制一次，当前房、已清空房和四类特殊房使用独立颜色。
- 新增 `RoomIcon2D.prefab` 与 `RoomConnection2D.prefab`，场景内建立 `ConnectionContainer` 和 `IconContainer`；两份 Prefab、容器层级及序列化引用均已核对。
- 地图可见性保留 `initVisible` Inspector 接口。当前版本选择默认展示完整拓扑；关闭后仍可使用“Start 起步、进入房间发现直接邻居”的探索显示逻辑，是否采用迷雾留待后续单独讨论。
- 正式场景已由 `Main_V1_5_2` 更新为 `Main_V1_5_3`，移动时保留原 `.meta` GUID，并同步更新 Build Settings；开发期 `LevelGraphDebugView` 已从正式场景移除，脚本继续保留供独立 Seed 验证使用。
- 用户已完成二维地图方向、连接、颜色、房间流程、强化、胜负结算、同 Seed/新 Seed 重开等整局 Play 验收；最终代码 review 未发现新的事件订阅、房间索引或序列化引用问题。
- 静态编译 `dotnet build Assembly-CSharp.csproj --no-restore --no-incremental` 为 **0 error / 0 项目代码 warning**（其余 warning 均来自 Unity Package）。V1.5-3 的 8 个步骤全部完成。
