# V1.5-3：程序化关卡生成 —— 连通地图、房间图、局内强化与完整单局

> V1 的第 5 周已经有了 `RoomConfig + Room + Door + LevelManager`，但它仍是一条固定数组：清完房间、碰门、传送到下一间。它证明了房间流程能跑，却还不是我们真正想要的 Roguelike 地图。
>
> V1.5-3 要把它升级成一张真正存在于世界里的地图：房间由实体通道连接，玩家可以直接走过去；地图有分岔、回头路、特殊房和 Boss；同一个 Seed 能稳定复现；清房后能获得只在本局生效的强化；死亡与胜利都能结算并重新开始。
>
> 本阶段拆成 8 步。每一步都能单独编译并 Play 验收，不要求一次把场景全部推倒重搭。完整参考实现位于 `Reference/Scripts/...`，你仍然在 `Assets/Scripts/...` 对应路径手动创建或替换。本文件对尚未创建的脚本和大规模重构文件给出完整代码；小范围改动至少以完整字段、属性或函数为单位给出。
...
开始新局（确定 Seed）
        │
        ▼
生成纯数据房间图 ── 校验失败则换派生 Seed 重试
        │
        ▼
实例化房间 + 实体通道 + 四向门洞
        │
        ▼
起始房 ─ 普通房 ─ 分岔房 ─ 普通房 ─ Boss 房
                     ├──────── 宝箱房
                     └──────── 精英房 ─ 恢复房
        │
        ├─ 进入战斗房：封门 → 清敌 → 三选一强化 → 开门
        ├─ 进入特殊房：领取奖励或恢复 → 开门
        ├─ 在通道中：跟随相机；进入房间：固定房间相机
        └─ 小地图只揭示已到达房间和相邻房间
                                      │
                    ┌─────────────────┴─────────────────┐
                    ▼                                   ▼
...
这次不是“随机摆几个房间”这么简单，而是同时完成四条闭环：

- **结构闭环**：图一定连通、没有重叠、Boss 足够远、特殊房数量正确，并且至少存在一个真正的分岔。
- **空间闭环**：数据图里的每条边都对应世界中的实体通道；没有邻居的方向用墙封死，有邻居才打开门洞。
- **玩法闭环**：进入房间、封门、清敌、领奖、重访、通关都能正确衔接。
- **单局闭环**：Seed、局内强化、死亡、胜利、统计和重新开始都属于同一次 Run。

---

## 0. 先冻结边界：哪些东西随机，哪些东西不随机
...
| `DungeonBuilder` | 把图变成房间和通道，选择 `RoomConfig` | 不决定战斗何时开始，不处理通关 |
| `Room + LevelManager` | 进入、封门、清房、奖励、相机、通关 | 不发明地图拓扑 |

这样拆的直接收益是：纯图生成可以批量验证 100 个 Seed，不必进入 Unity 物理世界；房间美术以后从灰盒换成 Tilemap，也不用重写随机算法。

### 0.2 首版使用“二维网格上的树”

首版地图故意不做环：每个新房间只连接到一个已存在房间，因此边数恒为 `房间数 - 1`。树的优势是：

- 不会出现两个生成分支抢占同一坐标后造成几何重叠；
- 不需要先解决通道交叉、锁匙门、回环捷径等额外问题；
- 每个房间的 `Depth` 就是它离起点的路径长度，Boss 和难度配置更直观；
- 已经能表达我们要的主路、岔路、宝箱房、精英房和恢复房。

环路是以后可以加的“额外边”，不是这一步的基础要求。

### 0.3 房间类型先用六种

| 类型 | 作用 | 是否建议为叶节点 |
|---|---|---|
| `Start` | 出生与教学缓冲，不刷敌 | 起点，不要求 |
| `Normal` | 常规战斗与主要强化来源 | 否 |
| `Elite` | 更难的敌人组合与更高风险 | 否 |
| `Treasure` | 无战斗或轻战斗，直接给强化 | 是 |
| `Recovery` | 首次进入恢复生命 | 是 |
| `Boss` | 最深叶节点，击败后胜利 | 是 |

这里不急着做 `Shop`。当前没有货币、售价、刷新和购买交互，如果现在硬塞商店，只会得到“叫商店的免费宝箱房”。恢复房在现有系统上有真实、可验收的行为，商店等以后拥有经济系统再加入。
...
- 房间数量、坐标、连接和房间类型；
- 每个房间选择到哪份 `RoomConfig`；
- 奖励三选一的抽取顺序。

但三条随机流不要共享同一个 `System.Random` 实例。参考实现分别用原始 Seed、`seed ^ 常量 A`、`seed ^ 常量 B` 创建拓扑、内容和奖励随机流。否则未来只是在某个房间多抽一次装饰，就可能把后面所有奖励一起改变。
...
### 2.1 新建 `Assets/Scripts/Level/RoomDirectionUtility.cs`
...
### 2.2 新建 `Assets/Scripts/Level/RoomNode.cs`
...
### 2.3 新建 `Assets/Scripts/Level/LevelGraph.cs`
...
### 2.4 新建 `Assets/Scripts/Level/LevelGenerationSettings.cs`
...
### 2.5 新建 `Assets/Scripts/Level/LevelGraphGenerator.cs`
...
### 2.6 新建 `Assets/Scripts/Level/LevelGraphValidator.cs`
...
### 2.7 新建 `Assets/Scripts/Debug/LevelGraphDebugView.cs`
...
1. `Create > Game > Level Generation Settings`，创建 `Assets/Data/LevelGenerationSettings.asset`。
...
### 3.1 新建 `Assets/Scripts/Level/RoomCatalog.cs`
...
### 3.2 新建 `Assets/Scripts/Level/RoomConnection.cs`
...
### 3.3 新建 `Assets/Scripts/Level/DungeonBuilder.cs`
...
### 3.4 大规模重写 `Assets/Scripts/Level/Room.cs`
...
### 4.1 重写 `Assets/Scripts/Level/Door.cs`
...
### 4.2 新建 `Assets/Scripts/Level/RoomEncounterTrigger.cs`
...
### 6.1 更新 `Assets/Scripts/Level/RoomConfig.cs`
...
Assets/Scripts/Debug/LevelGraphDebugView.cs
Assets/Scripts/Level/RoomDirectionUtility.cs
Assets/Scripts/Level/RoomNode.cs
Assets/Scripts/Level/LevelGraph.cs
Assets/Scripts/Level/LevelGenerationSettings.cs
Assets/Scripts/Level/LevelGraphGenerator.cs
Assets/Scripts/Level/LevelGraphValidator.cs
Assets/Scripts/Level/RoomCatalog.cs
Assets/Scripts/Level/RoomConnection.cs
Assets/Scripts/Level/RoomEncounterTrigger.cs
Assets/Scripts/Level/DungeonBuilder.cs
...
```text
Assets/Scripts/Level/Room.cs
Assets/Scripts/Level/LevelManager.cs
Assets/Scripts/Level/Door.cs
...
Assets/Scripts/Level/RoomConfig.cs
```
...
```text
Assets/Data/LevelGenerationSettings.asset
